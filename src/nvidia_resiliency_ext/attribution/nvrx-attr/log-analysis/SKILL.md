...
| `--model` | `nvdev/nvidia/llama-3.3-nemotron-super-49b-v1` | LLM model |
| `--temperature` | `0.2` | Sampling temperature |
| `--top_p` | `0.7` | Top-p nucleus sampling |
| `--max_tokens` | `8192` | Max output tokens |
| `--exclude_nvrx_logs` | off | Strip `nvidia_resiliency_ext` / `[workload:]` lines before chunking |
...
- `langchain-nvidia-ai-endpoints` and `logsage` packages installed
