...
compatibility: Requires SLURM cluster access, sbatch, NVIDIA_API_KEY, langchain-nvidia-ai-endpoints, logsage, nvidia-resiliency-ext installed.
metadata:
  author: nvidia
...
| `BASE_EXPERIMENTS_DIR` | `/home/sbak/experiments/llama4-scout-gb200` | Root for all output |
...
2. Calls `nvrx_logsage.py --output-format json` to get `restart_decision` and
   `attribution_text`
3. Calls `CollectiveAnalyzer` from `fr_attribution.py` to get `suspect_ranks`
4. Scores rank identification and fault-type classification for both sub-skills
5. Appends a scored row to `session_<timestamp>_report.md`
...
Scoring rubric applied automatically by `watch_and_analyze.sh`:

| Dimension | Pass criterion |
|---|---|
| **log_rank_ok** | Attribution text mentions `rank <N>` (or `process N`, `worker N`) matching injected rank |
| **log_type_ok** | Text contains hang keywords for GPU_SLEEP/LOCK_GIL; crash keywords for SIGKILL/SEGFAULT/etc. |
| **log_restart** | GPU_SLEEP/LOCK_GIL → `RESTART IMMEDIATE`; SIGKILL/SEGFAULT/GPU_ERROR → `STOP` |
| **fr_rank_ok** | `CollectiveAnalyzer.suspect_ranks` includes injected rank |

Manual scoring template (for additional dimensions):
```yaml
experiment:
  id:             <monotonic counter>
  fault_type:     GPU_SLEEP | GPU_ERROR | SIGKILL | …
  rank:           <injected rank>
  iter:           <injected iter>
  nodes:          <node count>
  job_id:         <SLURM_JOB_ID>
...
log_analysis:
  restart_decision:   <value>
  attribution_text:   <excerpt>
  rank_correct:       true | false | partial
  fault_type_correct: true | false
  iter_proximity:     pass | off_by_N
  notes:              <freeform>
...
```
| FAULT_TYPE | NODES | RANK | log_restart | log_rank_ok | log_type_ok | fr_rank_ok |
|------------|-------|------|-------------|-------------|-------------|------------|
| GPU_SLEEP  |   2   |  0   |   RESTART   |    false    |    true     |   partial  |
| GPU_SLEEP  |   2   |  1   |   RESTART   |    true     |    true     |    true    |
| GPU_ERROR  |   2   |  1   |    STOP     |   false     |   false     |    true    |
| SIGKILL    |   2   |  0   |    STOP     |   false     |   false     |    true    |
```
...
|---|---|
| `log_rank_ok=false` for rank-0 only | Rank-0 hang silences watchdog on other ranks; attr text lacks rank-0 signal |
| `log_type_ok=false` for all crash types | Crash-vs-hang keyword gap in `nvrx_logsage.py` fast-path |
| `log_restart` wrong for GPU_ERROR | LLM conflating GPU error with recoverable hang |
| `fr_rank_ok=partial` at large node counts | Wavefront analysis ambiguity at higher rank counts |
| `fr_rank_ok=no_dumps` | NCCL watchdog did not fire before job ended — adjust `TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC` |
...
## Appendix A: launch.sbatch fault parameters

`launch.sbatch` reads these env vars (set by `prepare_node_alloc.sh` via `--export`):
